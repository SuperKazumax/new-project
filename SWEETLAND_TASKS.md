...
## Task 7: 制御ゲートウェイを設計・実装する

### 目的

Webアプリから実機へ直接命令を出さず、間に安全確認用の制御ゲートウェイを置く。
...
- 新規 gateway 実装
- `sweetland-server/server.ps1`
- 将来の Raspberry Pi / サーボ制御コード

### やること

- まずMock gatewayを作る。
- WebサーバーからMock gatewayへ `left`、`right`、`scoop` を送れるようにする。
- safe / armed / emergency の3モードを作る。
- serverToken と adminToken を分ける。
- 緊急停止、連打制限、操作時間上限を入れる。
- 実機接続前にログだけで動作確認する。

### 受け入れ条件

- safe modeでは実機出力しない。
- armed modeでは有効な操作だけ通る。
- emergency modeでは全操作が拒否される。
- トークンなしの操作は拒否される。
- 連打は拒否される。
- すべての操作がログに残る。

## Task 8: Mock gateway実装を先に作る

### 目的

実機やサーボを動かさず、安全にWebサーバーと制御ゲートウェイの接続を確認する。
...
- `/health`、`/state`、`/command`、`/mode`、`/emergency-stop`、`/reset` を実装する。
- `serverToken` と `adminToken` を検証する。
- safe / armed / emergency modeを実装する。
- duration上限とcooldownを実装する。
- 操作ログを残す。
- WebサーバーからMock gatewayへ操作命令を送る。

### 受け入れ条件

- 実機なしでleft/right/scoopの送信確認ができる。
- safe modeでは `executed: false` になる。
- armed modeでは有効コマンドが `executed: true` になる。
- emergency modeでは全操作が拒否される。
- cooldown中の連打が拒否される。
- ログに結果が残る。
